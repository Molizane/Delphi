...
begin
  PostMessage(FPrincipal.Handle, WM_USER + 1000, tblCadastro.Fields[0].AsInteger, 0);
  Close;
end;

procedure TFrmModelo.smnFecharClick(Sender: TObject);
begin
  if FAcessoDireto then
    PostMessage(FPrincipal.Handle, WM_USER + 1000, 0, 0);
  Close;
end;
...
begin
  FPrincipal.Menu := FPrincipal.MainMenu;
  FPrincipal.pnlBotoes.Visible := True;
  Action := caFree;
end;
