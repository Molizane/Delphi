...
const
  Reserved = ';ORG;DA;DC;DD;DW;SUBLEQ;REL;';

var
  Curr, n, c, i: integer;
  nOrdem, nAdd, FBaseStart, FOffset, FBaseEnd: longword;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // Se já ocorreu geração de código (DA, DD, SUBLEQ), não pode mais ajustar labels
      ...
      begin
        ...
        begin
          FLines[i].Error := True;
          AddError(i, 'Invalid "ORG" address.');
        end
        else if nAdd < nOrdem then
        begin
          FLines[i].Error := True;
          AddError(i, 'Invalid "ORG" address. Address lesser than current.');
        end
        else
        begin
          nOrdem := nAdd;
    ...
    begin
      for i := 0 to Length(FLines) - 1 do
        try
          ...
          if SameText(FLines[i].Oper, 'DA') or SameText(FLines[i].Oper, 'DD') or SameText(FLines[i].Oper, 'subleq') then
          begin
