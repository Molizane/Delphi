...
/*       Victor Trucco @ 2013          */
/*                                     */
/* Contem o codigo do monitor em 0000h */
/* e o codigo da interface K7 em 0322h */
/*                                     */
/* Alguns bugs dos codigos foram       */
...
MENS:	db	CHR_M, CHR_E, CHR_S, CHR_T, CHR_0, CHR_R
...
/***************************************/
/*       TABELA DE FORMATOS HEXA       */
/***************************************/

INTD:	db	CHR_0, CHR_1, CHR_2, CHR_3, CHR_4, CHR_5, CHR_6, CHR_7
	db	CHR_8, CHR_9, CHR_A, CHR_B, CHR_C, CHR_D, CHR_E, CHR_F
...
ROTUL:	call	ATUAL
	...
	;ds	3, 0FFh		// 01C7 à 01C9 - NAO USADAS
...
/*****************************************/
/*  AJUSTE INICIAL VETOR DE INTERRUPCAO  */
/*****************************************/
...
TABCOM:	ds	16, 0
	db	high ROTPER, high ROTLT, high ROTUL, high ROTPR, high ROTRD, high ROTER, high RIV, high ROTCP, high ROTCD, high MAIN
	db	low ROTPER,  low ROTLT,  low ROTUL,  low ROTPR,  low ROTRD,  low ROTER,  low RIV,  low ROTCP,  low ROTCD,  low MAIN
	;ds	229, 0
...
FINAL:	ld	hl, BUF+4
GERAL:	ld	a, CHR_APAG				// Apaga 2 ultimos displays
	ld	(DISP+4), a
	ld	(DISP+5), a
	ld	a, b
	push	hl
...
GFITA:	ld	a, 0FFh					// seta todos os displays
	out	(PDIG), a
...
CBYTES:	call	PARAM					// vamos calcular os parametros
	ret	c					// se end. final for menor que end. inicial deu erro, retornamos
	xor	a					// zeramos A
SOMA:	add	a, (hl)					// somamos um byte
...
PARAM:	ld	hl, BUF+2				// aponta para end. inicial
...
SAFITA:	ld	e, (hl)					// pegamos um byte dos dados
...
SABYTE:	ld	d, 8					// vamos gerar 8 bits
	or	a					// bit de start eh 0
	call	SAIBIT					// geramos bit de start
LOOPC1:	rr	e					// vamos enviar o bit LSB primeiro
...
SAIBIT:	exx						// salva registradores
	ld	h, 0					// zera H
	jr	c, SAI1					// se bit a ser gerado for 1, pulamos
SAI0:	ld	l, ZERO2K				// gerar bit 0: 8 ciclos de 2KHz
	call	GE2KHZ
	ld	l, ZERO1K				// e 2 ciclos de 1KHz
	jr	FINAL2					// pulamos

SAI1:	ld	l, UM2K					// gerar bit 1: 4 ciclos de 2KHz
	call	GE2KHZ
	ld	l, UM1K					// e 4 ciclos de 1KHz
FINAL2:	call	GE1KHZ					// geramos o sinal de 1KHz
...
GE1KHZ:	ld	c, V1KHZ				// gerar tom de 1KHz
	jr	TOM

GE2KHZ:	ld	c, V2KHZ				// gerar tom de 2KHz
TOM:	add	hl, hl					// duplicamos a quantidade de ciclos para termos
	ld	de, 1					// a quantidade de semiciclos
	ld	a, 0FFh					// enviamos primeiro semiciclo positivo (PE0 = 1)
QUAD:	out	(PPS), a				// setamos PE0
...
LFITA:	ld	a, CHR_APAG				// Apaga 2 ultimos digitos
	ld	(DISP+4), a
	ld	(DISP+5), a
...
COND2:	call	PERIOD					// Mede tamanho do ciclo
...
COND3:	call	PERIOD					// aqui temos 1 segundo de sinc. 1KHz estavel
...
PERIOD:	ld	de, 0					// Contador de ciclos
LOOP3:	in	a, (PPE)				// le porta K7 (bit 0)
	inc	de					// incrementa contador
	rr	a					// joga bit lido (bit 0) para o carry
	jr	c, LOOP3				// enquanto estiver recebendo 1 volta
	xor	a	; ld	a, 00h			// envia 0 para o auto-falante de monitoramento
	out	(PPS), a
LOOP4:	in	a, (PPE)				// le porta K7 (bit 0)
	inc	de					// incrementa contador
	rr	a					// joga bit lido (bit 0) para o carry
...
ENFITA:	xor	a					// zera A
	ex	af, af					// salva A
LOOP5:	call	LEBYTE					// lemos 1 byte
...
LEBYTE:	call	LEBIT					// lemos o bit de start (0)
	ld	d, 8					// vamos ler 8 bits
LOOP6:	call	LEBIT					// ler o bit (vem no carry)
...
LEBIT:	exx						// salvamos os registradores
	ld	hl, 0					// contador do que?
LOOP7:	call	PERIOD					// calculamos o tamanho do ciclo, carry indica se é 1KHz (C=0) ou 2KHz (C=1)
...
PERR:	inc	l					// temos um pulso de 2KHz, incrementamos L
...
ERROT:	ex	af, af					// trocamos AF por AF'
	scf						// setamos o carry em AF'
	ex	af, af					// voltamos de AF' para AF
	exx						// restauramos os regs. salvos anteriormente
	ret						// e retornamos

MERRO:	ld	hl, ERRO				// houve erro, exibimos a mensagem no display
	call	REST					// eternamente
	jr	MERRO

MFIM:	ld	hl, FIM					// acabou o procedimento, exibimos "FIN" no display
	call	REST					// eternamente
	jr	MFIM

ERRO:	db	CHR_E, CHR_R, CHR_R, CHR_0, CHR_APAG, CHR_APAG
FIM:	db	CHR_F, CHR_I, CHR_M, CHR_APAG, CHR_APAG, CHR_APAG
TITULO:	db	CHR_T, CHR_I, CHR_T, CHR_V, CHR_L, CHR_0
MFONTE:	db	CHR_F, CHR_0, CHR_M, CHR_T, CHR_E, CHR_APAG
MFINAL:	db	CHR_F, CHR_I, CHR_M, CHR_A, CHR_L, CHR_APAG
